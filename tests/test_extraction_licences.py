...
def test_chargement_et_anomalies():
    lancer()
    anomalies = sql("SELECT login, code_anomalie FROM dss_licences.fait_anomalie_jour")
    ...
    licences = sql("SELECT login, code_ads, type_licence FROM dss_licences.fait_licence_jour")
    ...
    vue = dict(sql("SELECT login, est_inactif FROM dss_licences.v_qlik_utilisateur_jour"))
    assert vue == {"alice": 0, "bob": 0, "carol": 1, "dave": 0, "eve": 1, "frank": 0}
    assert sql("SELECT jours_sans_connexion FROM dss_licences.fait_utilisateur_jour "
               "WHERE login = 'carol'") == [(200,)]

    agg = sql("SELECT nb_comptes, nb_comptes_actifs, nb_actifs_inactifs FROM "
              "dss_licences.agg_licence_jour WHERE code_ads = 'AAE_CSDIA' AND type_licence = 'DESIGNER'")
...
def test_relance_meme_jour_idempotente():
    lancer()
    lancer()
    assert sql("SELECT count(*) FROM dss_licences.fait_utilisateur_jour") == [(6,)]
    assert sql("SELECT count(*) FROM dss_licences.fait_anomalie_jour") == [(7,)]
...
def test_purge_rgpd_conserve_agregat():
    ...
    sql("INSERT INTO dss_licences.dim_utilisateur VALUES ('parti', 'Parti', NULL, NULL, NULL, %s, %s)",
        (ancienne, ancienne))
    sql("INSERT INTO dss_licences.fait_utilisateur_jour VALUES (%s, 'parti', 'DESIGNER', true, "
        "NULL, true, 10, 0, 0, 1)", (ancienne,))
    sql("INSERT INTO dss_licences.agg_licence_jour VALUES (%s, 'AAE_CSDIA', 'DESIGNER', 1, 1, 0, 1, 1, 90)",
        (ancienne,))
    lancer()
    assert sql("SELECT count(*) FROM dss_licences.dim_utilisateur WHERE login = 'parti'") == [(0,)]
    assert sql("SELECT count(*) FROM dss_licences.fait_utilisateur_jour WHERE date_extraction = %s",
               (ancienne,)) == [(0,)]
    assert sql("SELECT count(*) FROM dss_licences.agg_licence_jour WHERE date_extraction = %s",
               (ancienne,)) == [(1,)]
